...
const App: React.FC = () => {
  ...
  // 애니메이션 시퀀스 수정: 매 페이즈(0~3)가 끝날 때마다 로고(4)를 표시
                    ...
                    <div className="w-full max-w-[340px] mb-8 border-2 border-gray-800 p-2">
                      <table className="w-full text-[8px] leading-relaxed">
                        ...
                        <tbody>{(() => {
                          ...
                          const rank = idx + 1;
                          let data = rank <= 7 ? leaderboard.slice(0, 7).map((e, i) => ({ e, r: i + 1, d: false })) : [...leaderboard.slice(0, 5).map((e, i) => ({ e, r: i + 1, d: false })), { e: null, r: 0, d: true }, { e: leaderboard[idx], r: rank, d: false }];
                    ...
                    <div className="flex justify-center items-center w-full max-w-[340px] mb-10 mt-4 px-2">
                      <div className="grid grid-cols-3 w-full">
                        {[0, 1, 2].map((iconIdx, i) => {
                          ...
                          // '걸음' 아이콘(index 1)의 뒷면 스케일인 70 * 0.9 = 63px로 통일
                          const backSize = 63;
